...
class Tenant:
    __slots__ = (
        "_id", "_version", "_name", "_slug", "_status", "_metadata",
        "_created_at", "_updated_at", "_pending_events",
    )

    def __init__(self, *, tenant_id: TenantId, version: int, name: str, slug: str,
                 status: TenantStatus, created_at: datetime, updated_at: datetime,
    ...
    @classmethod
    def create(cls, *, name: str, slug: str, created_at: datetime,
               metadata: Mapping[str, object] | None = None) -> "Tenant":
        tenant = cls(tenant_id=TenantId.new(), version=0, name=name, slug=slug,
                     status=TenantStatus.PENDING, created_at=created_at,
                     updated_at=created_at, metadata=metadata)
        tenant._record(TenancyEventType.TENANT_CREATED, created_at)
        return tenant

    @classmethod
    def _rehydrate(cls, *, tenant_id: TenantId, version: int, name: str, slug: str,
                   status: TenantStatus, created_at: datetime, updated_at: datetime,
                   metadata: Mapping[str, object]) -> "Tenant":
        return cls(tenant_id=tenant_id, version=version, name=name, slug=slug,
                   status=status, created_at=created_at, updated_at=updated_at,
                   metadata=metadata)

    @property
    def id(self) -> TenantId: return self._id
    @property
    def version(self) -> int: return self._version
    @property
    def name(self) -> str: return self._name
    @property
    def slug(self) -> str: return self._slug
    @property
    def status(self) -> TenantStatus: return self._status
    @property
    def created_at(self) -> datetime: return self._created_at
    @property
    def updated_at(self) -> datetime: return self._updated_at
    @property
    def metadata(self) -> Mapping[str, object]: return self._metadata

    def activate(self, *, at: datetime) -> None:
        self._transition(TenantStatus.PENDING, TenantStatus.ACTIVE, "activate", at,
                         TenancyEventType.TENANT_ACTIVATED)

    def suspend(self, *, at: datetime) -> None:
        self._transition(TenantStatus.ACTIVE, TenantStatus.SUSPENDED, "suspend", at,
                         TenancyEventType.TENANT_SUSPENDED)

    def reactivate(self, *, at: datetime) -> None:
        self._transition(TenantStatus.SUSPENDED, TenantStatus.ACTIVE, "reactivate", at,
    ...
    def archive(self, *, at: datetime) -> None:
        self._transition(TenantStatus.DISABLED, TenantStatus.ARCHIVED, "archive", at,
    ...
    def _transition(self, expected: TenantStatus, target: TenantStatus, action: str,
    ...
    def _set_status(self, status: TenantStatus, at: datetime,
